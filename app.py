...
<style>
    ...
    .currency-card { background-color: #262730; padding: 10px; border-radius: 10px; border: 1px solid #41444b; margin-bottom: 10px; text-align: center; }
</style>
...
def calculate_portfolio(df_trans, df_prices):
    ...
    for v, d in port.items():
        ...
        rows.append({"Grup": d["tur"], "Varlık": v, "Adet": d["adet"], "Fiyat": cp, "Maliyet": d["maliyet"], "Net Değer": val - vergi, "Net Kâr": (val - vergi) - d["maliyet"], "Vergi": vergi})
        tot_w += (val - vergi)
        tot_t += vergi
...
def main():
    ...
    if df_prices.empty: st.warning("Fiyat verisi bulunamadı."); return

    with st.sidebar:
        st.header("💎 Varlık Paneli")
        last = df_prices.iloc[-1]
        usd, eur = last["DOLAR KURU"], last["EURO KURU"]
        st.markdown(f'<div class="currency-card">🇺🇸 USD: {usd:.2f} ₺<br>🇪🇺 EUR: {eur:.2f} ₺</div>', unsafe_allow_html=True)
    ...
    if page == "Portföyüm":
        ...
        for i, (tab, curr, rate) in enumerate(zip(tabs, ["TL", "USD", "EUR"], [1.0, usd, eur])):
            with tab:
                ...
                # Pasta Dilimi ve Dağılım (Unique Key eklendi)
                ...
                with col_p1:
                    ...
                    st.plotly_chart(fig_p, use_container_width=True, key=f"pie_{curr}_{i}")
                ...
                with col_p2:
                    ...
                    st.plotly_chart(fig_b, use_container_width=True, key=f"bar_{curr}_{i}")

                st.subheader("📋 Varlık Detayı")
                st.dataframe(df_view.style.format({"Net Değer": "{:,.2f}"}), use_container_width=True)
