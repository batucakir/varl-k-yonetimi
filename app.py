    ...
    for _, row in df_trans.iterrows():
        ...
        if isl == "ALIS":
            ...
            if port[var]["adet"] > 0:
                ...
                port[var]["maliyet"] -= (ad * avg); port[var]["adet"] -= ad
...
def render_rebalance_assistant(df_view):
    ...
    target_ratios = {}
    for i, row in df_grp.iterrows(): target_ratios[row["Grup"]] = cols[i].number_input(f"Hedef % ({row['Grup']})", 0, 100, int(100/len(df_grp)))
    analysis = []
    for i, row in df_grp.iterrows():
        ...
        analysis.append({"Grup": row["Grup"], "Mevcut Değer": row["Net Değer"], "Mevcut Oran": f"%{(row['Net Değer']/total_val*100):.1f}", "Aksiyon": aks})
...
def main():
    ...
    if page == "Portföyüm" and not df_trans.empty and not df_prices.empty:
        ...
        for t, curr, rate in [(tab1, "TL", 1.0), (tab2, "$", usd_rate), (tab3, "€", eur_rate)]:
            with t:
                ...
                with c1:
                    ...
                    # ALTIN İÇİN ÖZEL SARI RENK
                    color_map = {"ALTIN": "#FFD700", "FON": "#2ca02c", "HİSSE": "#d62728", "NAKİT": "#1f77b4"}
                    fig_p = px.pie(df_p, values="Net Değer", names=df_p.columns[0], hole=0.4, color=df_p.columns[0] if grp_mode == "Ana Gruplar" else None, color_discrete_map=color_map if grp_mode == "Ana Gruplar" else None)
                    fig_p.update_traces(textinfo="percent+label"); st.plotly_chart(fig_p, use_container_width=True)
                with c2:
                ...
                st.dataframe(df_show.style.format({"Adet": "{:,.0f}", "Fiyat": "{:,.4f}", "Maliyet": "{:,.2f}", "Net Değer": "{:,.2f}", "Net Kâr": "{:,.2f}", "Vergi": "{:,.2f}", "Kâr %": "%{:,.2f}"}), use_container_width=True, hide_index=True)
                if curr == "TL": st.divider(); render_rebalance_assistant(df_view)
                st.divider(); st.subheader("🥇 Altın Makas"); last_p = df_prices.iloc[-1]; gold_cols = st.columns(4)
                for i, (name, key) in enumerate([("Gram", "GRAM ALTIN"), ("Ata", "ATA ALTIN"), ("22 Ayar", "22 AYAR ALTIN"), ("Çeyrek", "ÇEYREK ALTIN")]):
                    ...
                    gold_cols[i].metric(name, format_tr_money(satis), f"Makas: {format_tr_money(satis-alis)}", delta_color="inverse")
        ...
        def color_change(val): return f"color: {'#00FF00' if val > 0.0001 else '#FF4B4B' if val < -0.0001 else 'white'}; font-weight: bold"
        col_cfg = {"Varlık": st.column_config.TextColumn("Varlık", width="small"), "Fiyat": st.column_config.NumberColumn("Fiyat", format="%.4f TL"), "RSI": st.column_config.NumberColumn("RSI", format="%.0f"), "Trend": st.column_config.LineChartColumn("Trend", y_min=0, width="small")}
        for k in ivs.keys(): col_cfg[f"{k} Değişim"] = st.column_config.NumberColumn(f"{k} Değişim", format="%.2f %%")
        ...
        def show_table(keyword):
            ...
            if not df_s.empty: st.dataframe(df_s.style.applymap(color_change, subset=[c for c in df_s.columns if "Değişim" in c]), column_config=col_cfg, use_container_width=True, hide_index=True)
