...
def load_data():
    try:
        ...
        if len(data_prices) > 1:
            ...
            # --- AKILLI FORMAT ONARICI (V63) ---
            df_prices['Tarih'] = pd.to_datetime(df_prices['Tarih'], errors='coerce')
            
            for col in df_prices.columns:
                if col != "Tarih":
                    # Önce noktayı kaldır, sonra virgülü noktaya çevir (7.408,21 -> 7408.21)
        ...
        try:
            ...
            if len(data_trans) > 1:
                ...
                # İşlemler sayfası format onarıcı
                for c in ['Adet', 'Fiyat']:
...
def get_pct_change(df, col, days):
    ...
    # V63: Dakika yerine gün bazlı arama (Geçmiş veri için)
    target_date = df.iloc[-1]['Tarih'] - timedelta(days=days)
    temp_df = df[df['Tarih'] <= target_date]
    if temp_df.empty: 
        old_price = df.iloc[0][col]
    else:
        old_price = temp_df.iloc[-1][col]
...
def calculate_portfolio(df_trans, df_prices):
    ...
    for v, d in port.items():
        ...
        nd = gd - vergi
        table_rows.append({"Grup": d["tur"], "Varlık": v, "Adet": d["adet"], "Fiyat": cp, "Maliyet": d["maliyet"], "Net Değer": nd, "Net Kâr": nd - d["maliyet"], "Vergi": vergi})
        tot_w += nd
        tot_t += vergi
...
def prepare_historical_trend(df_prices, df_trans, asset_map, rate=1.0):
    ...
    # V63: Günlük bazda temizleme
    df_prices['Tarih_Day'] = df_prices['Tarih'].dt.date
    daily_prices = df_prices.groupby('Tarih_Day').last().reset_index()
    
    trend_data = []
    for _, pr in daily_prices.iterrows():
        cd = pd.to_datetime(pr['Tarih_Day'])
        temp_trans = df_trans[df_trans['Tarih'] <= cd]
        running_port = {}
        for _, tr in temp_trans.iterrows():
            v, isl, ad = str(tr['Varlık']).strip(), str(tr['İşlem']).upper().strip(), float(tr['Adet'])
            running_port[v] = running_port.get(v, 0) + (ad if isl == "ALIS" else -ad)
        ...
        for v, qty in running_port.items():
            ...
            for col in daily_prices.columns:
                if sv in col: price = pr[col]; break
            if v == "TL Bakiye": price = 1.0
            tot += (qty * price)
...
def render_benchmark_chart(df_trend, df_prices):
    ...
    fig.update_layout(title="🏆 Performans Kıyaslama (1 Yıllık % Getiri)", yaxis_title="Getiri (%)", hovermode="x unified", height=450, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
...
def render_rebalance_assistant(df_view):
    ...
    for i, row in df_grp.iterrows():
        target_ratios[row["Grup"]] = cols[i].number_input(f"Hedef % ({row['Grup']})", min_value=0, max_value=100, value=int(100/len(df_grp)))
    if sum(target_ratios.values()) != 100: st.warning(f"⚠️ Hedef toplamı %100 olmalı (Şu an %{sum(target_ratios.values())})")
    analysis = []
    for i, row in df_grp.iterrows():
        ...
        analysis.append({"Grup": row["Grup"], "Mevcut Değer": row["Net Değer"], "Mevcut Oran": f"%{row['Mevcut Oran']:.1f}", "Hedef Oran": f"%{target_ratios[row['Grup']]:.1f}", "Fark (TL)": fark_tl, "Aksiyon": tavsiye})
    df_ana = pd.DataFrame(analysis)
    def style_aks(val): return f"color: {'#00FF00' if 'AL' in val else '#FF4B4B' if 'SAT' in val else 'white'}; font-weight: bold"
    st.dataframe(df_ana.style.applymap(style_aks, subset=['Aksiyon']).format({"Mevcut Değer": "{:,.2f} TL", "Fark (TL)": "{:,.2f} TL"}), use_container_width=True, hide_index=True)
...
def main():
    ...
    if page == "Portföyüm":
            ...
            for t, curr, rate in [(tab1, "TL", 1.0), (tab2, "$", usd), (tab3, "€", eur)]:
                with t:
                    if not df_view.empty:
                        ...
                        net_p = df_view["Net Kâr"].sum() / rate
                        c1, c2, c3 = st.columns(3)
                        c1.metric("Toplam Varlık", f"{format_tr_money(tot_w/rate)} {curr}", f"Vergi: -{format_tr_money(tot_t/rate)} {curr}", delta_color="inverse")
                        c2.metric("Net Kâr", f"{format_tr_money(net_p)} {curr}")
                        c3.metric("Kâr Oranı", f"%{format_tr_money((net_p/(df_view['Maliyet'].sum()/rate)*100) if df_view['Maliyet'].sum()>0 else 0)}")
                        st.divider()
                        if curr == "TL":
                            st.subheader(f"🎯 Hedef: {format_tr_money(HEDEF_SERVET_TL)} TL")
                            st.progress(min(tot_w/HEDEF_SERVET_TL, 1.0))
                            h1, h2 = st.columns(2)
                            h1.caption(f"🏁 Kalan: **{format_tr_money(HEDEF_SERVET_TL - tot_w)} TL**")
                        ...
                        if not df_trend.empty:
                            ...
                            fig.update_layout(yaxis_range=[df_trend["Toplam Servet"].min()*0.95, df_trend["Toplam Servet"].max()*1.05], height=400, hovermode="x unified")
                        ...
                        with c1:
                            ...
                            df_p = df_view.groupby("Grup" if grp_mode == "Ana Gruplar" else "Varlık")["Net Değer"].sum().reset_index()
                            fig_p = px.pie(df_p, values="Net Değer", names=df_p.columns[0], hole=0.4, color_discrete_sequence=px.colors.qualitative.Prism if grp_mode == "Detaylı" else ["#FFD700", "#1f77b4", "#2ca02c", "#d62728"])
                            fig_p.update_traces(textinfo="percent+label", textfont_size=16); st.plotly_chart(fig_p, use_container_width=True, key=f"pie_{curr}")
                        with c2:
        ...
        if not df_prices.empty:
            market_data = []
            # V63: Oranları GÜN bazlı hesapla
            ivs = {"1 Gün": 1, "1 Hafta": 7, "1 Ay": 30, "6 Ay": 180, "1 Yıl": 365}
            for col in df_prices.columns:
                if col in ["Tarih", "Tarih_Day"]: continue
                if any(x in col for x in ["FİYAT", "ALTIN", "KURU"]):
                    series = df_prices[col].dropna()
                    if series.empty: continue
                    row = {"Varlık": col.replace(" FİYAT", "").replace(" ALIŞ", "").replace(" SATIŞ", ""), "Fiyat": series.iloc[-1], "RSI": calculate_rsi(series).iloc[-1], "Trend": series.tail(30).tolist()}
                    for label, days in ivs.items(): row[label] = get_pct_change(df_prices, col, days)
                    market_data.append(row)
            df_m = pd.DataFrame(market_data)
            st.dataframe(df_m.style.format({"Fiyat": "{:,.4f}", "RSI": "{:,.0f}", "1 Gün": "{:.2%}", "1 Hafta": "{:.2%}", "1 Ay": "{:.2%}", "6 Ay": "{:.2%}", "1 Yıl": "{:.2%}"}), column_config={"Trend": st.column_config.LineChartColumn("30 Kayıt")}, use_container_width=True, hide_index=True)
